...
class BatchedCCM:
	...
	def CrossMap(self, X, Y, embedDims, maxDims):
		...
		libraryIndices, _, _ = BuildEmbeddingIndices(data = X, columns = numpy.arange(numSources).tolist(), train = self.train, test = self.test, embedDimensions = dims,
		                                             predictionHorizon = self.predictionHorizon, step = self.step, exclusionRadius = self.exclusionRadius,
		                                             embedded = self.embedded, validLib = self.validLib)
