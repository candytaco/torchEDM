...
class SMapFitter(EDMFitter):
	"""
	sklearn-compatible wrapper for S-Map (Sequential Locally Weighted Global Linear Maps).

	Usage::

	    fitter = SMapFitter(EmbedDimensions=4, Theta=2.0)
	    fitter.fit(X_train, y_train)
	    y_pred = fitter.predict(X_test)
	    result = fitter.result_  # full SMapResult (includes coefficients)
	"""
	...
	def fit(self,
			...
			TrainStart: int = 0,
			TrainEnd: int = 0,
			trainTime: Optional[numpy.ndarray] = None) -> 'SMapFitter':
		"""
	...
	def predict(self,
		...
		"""
		Predict using S-Map on the supplied test points.
