"""Greedy variable-selection path with the convergence check disabled: dimx noCCM vs
...
(TS33, TS4, TS31, TS56, TS17, TS68, TS48, TS76) with correlations equal to 6 decimals;
spectra for dims 2..8 agree to ~5e-7 (the reference stores correlations as float32);
...
def main():
    ...
    torch_correlation = [r for r in result.performance[0] if not np.isnan(r)]

    print(f'{"dim":>3} {"ref":>7} {"corr":>9}   {"torch":>7} {"corr":>9}  match')
    for d in range(D):
        print(f'{d+1:>3} {ref_vars[d]:>7} {ref_correlation[d]:>9.6f}   '
              f'{torch_vars[d]:>7} {torch_correlation[d]:>9.6f}  '
