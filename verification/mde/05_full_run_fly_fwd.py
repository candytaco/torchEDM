...
Reference: the package's own Fly FWD validation config (D=8, training window [1,300],
test window [301,600], crossMapRhoMin=0.2, embedDimRhoMin=0.65, ccmSlope=0.01,
ccmSeed=7777) — reproduces MDE_Fly_2_Valid.csv.

torchEDM (post-alignment): MaxD=8, matched windows, MinPredictionThreshold=0.2
(~crossMapRhoMin), MinCandidatePerformance=0.65 (~embedDimRhoMin), convergence-check
...
def torch_run(df, ts_cols, convergent):
    ...
    correlation = [float(r) for r in result.performance[0] if not np.isnan(r)]
    return sel, correlation
...
def main():
    ...
    ref_correlation = list(mde_ref.MDEOut['rho'])

    sel_post, correlation_post = torch_run(df, ts_cols, 'post')
    sel_pre, correlation_pre = torch_run(df, ts_cols, 'pre')

    print(f'{"dim":>3} | {"reference":>9} {"corr":>7} | {"torch post":>10} {"corr":>7} | {"torch pre":>9} {"corr":>7}')
    for d in range(max(len(ref_vars), len(sel_post), len(sel_pre))):
        ...
        print(f'{d+1:>3} | {f(ref_vars,d):>9} {g(ref_correlation,d):>7} | '
              f'{f(sel_post,d):>10} {g(correlation_post,d):>7} | '
              f'{f(sel_pre,d):>9} {g(correlation_pre,d):>7}')
